...
## 使用說明
1. **執行模式**: 必須使用背景非同步執行。
17|2. **指令格式**: `python3 ~/.hermes/skills/mlops/comfy_t2i/scripts/t2i_engine.py [workflow_path] "[prompt]" --user_id [telegram_id]`
3. **Prompt 注入規則**:
...
2. 發送工作流到 ComfyUI
3. 輪詢生成狀態直到完成
31|4. 下載圖片到 `~/.hermes/hermes-agent/output/t2i_transient_{user_id}_{timestamp}_{pid}.png`
...
## 執行指令範例
```bash
python3 ~/.hermes/skills/mlops/comfy_t2i/scripts/t2i_engine.py ~/.hermes/skills/mlops/comfy_t2i/references/01-Flux2-Klein-T2I.json "A beautiful sunset over the ocean" --user_id 995681855
```
